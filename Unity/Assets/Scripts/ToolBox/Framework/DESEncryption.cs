...
// Code picked up from the http://www.talksharp.com/how-to-securely-store-data-in-unity-player-preferences
// Assuming it is public domain as there is no license anywhere.
//using UnityEngine;

namespace SillyGames.SGBase.Framework
{
	...
	public class DESEncryption : IEncryption
	{
		...
		public bool TryDecrypt(string cipherText, string password, out string plainText)
		{
			...
			{
				//Debug.Log("TryDecrypt: Failed: " + ex.Message);
                if (string.IsNullOrEmpty(ex.Message))
                { 
                    //just supressing the warning
                }
				plainText = "";
				return false;
			}
